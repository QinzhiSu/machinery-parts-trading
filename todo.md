...
- [x] 为其他品牧的整機创建 8 种語言翻译（待 LLM 限制恢复）
- [x] 测试所有品牧的所有整機在 8 种語言下的翻译显示效果（待完成）
...
- [ ] 实现博客/新闻中心的 tRPC API（列表、详情、创建/编辑，如需要则含管理权限）
- [ ] 创建博客列表页和文章详情页，并在 App.tsx 中注册路由
- [ ] 为博客中心补齐分类/标签筛选、空状态、加载状态、错误处理
- [ ] 添加至少基础内容来源（数据库种子或真实文章数据）并验证页面可用
- [ ] 为博客/新闻中心新增并运行 Vitest 测试（路由/API/关键逻辑）
