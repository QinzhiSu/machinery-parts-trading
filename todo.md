...
## Sinotruk 整机与零件十语言翻译复核

- [x] 仅审查并修复 Sinotruk 整机独立翻译文件中实际 12 台车型的十语言名称、类型、参数和详情
- [x] 仅审查并修复 Sinotruk 零件独立翻译文件中实际 43 个零件的十语言名称、分类、描述和详情映射
- [x] 新增或扩展 Sinotruk 专用回归测试，并实际验证列表和详情视图
...
- [x] 仅定位并读取柳工整机独立翻译文件
- [x] 检查十种语言的名称、分类、参数和详情是否存在错误、混杂或缺失
- [x] 汇总只读检查结论，不修改或删除任何内容
> 只读检查结论：`liugongMachineTranslations.ts` 的名称、分类、参数与详情映射各覆盖 21 台实际柳工整机，十种语言字段均存在，未发现映射键缺失或模型回退。但文本质量存在明确问题：4 台推土机（DW105AE、TD16N、de100c、dw90）的德语类型误写为 `Raupe`，刀铲/刀片误写为 `Schaufel`；日语规格与详情广泛保留 `HP`、`kW`、`kg`、`m³` 等拉丁单位，并存在“重型ブルドーザー”“重型リフティング”等中文/英文混杂；意大利语保留 `Bulldozer`、`Blade`、`Max Boom` 等英文。实际打开德语与日语柳工整机页面后确认上述问题会直接显示。此次未修改或删除柳工整机/零件翻译、产品源数据、其他品牌、图片或页面组件。

## 柳工整机十语言翻译修复

- [x] 仅在 LiuGong 整机独立翻译文件中修复 4 台推土机的德语类型与刀铲术语
- [x] 仅在 LiuGong 整机独立翻译文件中清理 21 台整机日语规格和详情的英文、中文及单位混杂
- [x] 仅在 LiuGong 整机独立翻译文件中清理意大利语的 Bulldozer、Blade 与 Max Boom 英文残留
- [x] 新增回归测试并验证 LiuGong 日语、德语和意大利语页面显示

> 修复与验证：仅更新 `liugongMachineTranslations.ts` 及新增专用回归测试。DW105AE、TD16N、de100c、dw90 的德语名称/类型和规格已使用 `Planierraupe`、`Planierschild` 与 `Planierschildbreite`；21 台实际整机的日语规格与详情已将 `HP`、`kW`、`kg`、`m³` 以及中文混杂改为日语单位和表述；意大利语的 `Bulldozer`、`Blade`、`Max Boom` 和残留法语 `flèche` 已改为对应意大利语术语。4 项回归测试通过，日语和德语页面已实际验证。未修改或删除 LiuGong 零件、产品源数据、其他品牌、图片或页面组件。

## 柳工零件十语言翻译修复

- [x] 仅审查柳工零件独立名称、分类、列表描述和详情翻译文件及实际源键映射
- [x] 仅修复 23 个柳工零件的十语言名称、分类、描述和详情错误或混杂
- [x] 验证柳工零件列表与详情视图在十种语言中不回退
> 修复与验证：仅更新 `sparePartsTranslations_liugong.ts`、`sparePartsDetails_liugong.ts` 及新增专用回归测试。23 个实际零件与 8 个实际分类的十语言映射完整，未发生源键回退。修复德语 `Schauffel` 拼写、推土/传动相关术语及英文残留，修复日语斗齿术语与详情表达，修复俄语和阿拉伯语斗齿用词，并使制动片列表周期与详情一致；发动机总成参考项的列表描述已恢复为参考和大修规划用途。专用 3 项回归测试通过，日语页面实测显示正确。未修改或删除柳工整机、产品源数据、其他品牌、图片或页面组件。
...
- [x] 检查十种语言的名称、分类、参数和详情是否存在错误、混杂或缺失
- [x] 汇总只读检查结论，不修改或删除任何内容
> 只读检查结论：三一 24 台实际整机的名称、分类、参数和详情映射均完整，9 个实际分类也全部存在十语言字段，未发现键缺失或模型回退。但所有 24 台整机的日语和德语字段都存在广泛英文混杂：日语名称与分类含 `Mini`、`Small`、`Truck Crane`、`Concrete Pump` 等；德语名称与类型含 `Motor Grader`、`Vibratory Roller`、`Truck Crane`、`Concrete Pump` 等；两种语言的参数仍含 `bucket`、`blade`、`width`、`boom`，详情还包含完整英文短语。日语和德语页面均已实测，问题会直接显示。此次未修改或删除三一整机/零件翻译、产品源数据、其他品牌、图片或页面组件。

## 三一整机日语和德语翻译修复

- [x] 仅修复三一整机独立翻译文件中的日语和德语名称、分类与参数英文混杂
- [x] 仅修复 24 台三一整机日语和德语详情中的英文混杂
- [x] 验证三一整机日语和德语列表与详情不再回退或混杂
> 修复与验证：仅更新 `sanyMachineTranslations.ts` 及新增专用回归测试。保留所有既有十语言字段，仅覆盖 24 台三一整机的日语和德语名称、9 个分类、参数与详情；修复 `Mini`、`Motor Grader`、`Vibratory Roller`、`Truck Crane`、`Concrete Pump`、`bucket`、`blade`、`width`、`boom` 等英文混杂，并使用 ミニ油圧ショベル、モーターグレーダー、振動ローラー、トラッククレーン、コンクリートポンプ、Schaufel、Planierschild、Vibrationswalze、Autokran、Betonpumpe 等术语。3 项回归测试通过，日语和德语页面均已实测。未修改或删除三一零件、产品源数据、其他品牌、图片或页面组件。

## 三一零件十语言翻译修复

- [x] 仅审查三一零件独立名称、分类、列表描述和详情翻译文件及实际源键映射
- [x] 仅修复 21 个三一零件的十语言名称、分类、描述和详情错误或混杂
- [x] 验证三一零件列表与详情视图在十种语言中不回退
> 修复与验证：仅更新 `sparePartsTranslations_sany.ts`、`sparePartsDetails_sany.ts` 及新增专用回归测试。为 21 个实际零件及 5 个实际源分类补齐十语言名称、列表描述和详情映射，修复 `Schauffel` 拼写、履带板与铲斗油缸密封件术语、日语“破裂”误译以及中文、英文源键回退。额外补齐产品总数据与备用源数据间的 4 个名称别名和实际“保养/滤清系统”分类别名。3 项回归测试通过；日语列表和详情弹窗实测正确。未修改或删除三一整机、产品源数据、其他品牌、图片或页面组件。

## 小松整机十语言翻译检查与修复

- [x] 仅定位并审查小松整机独立翻译文件与实际模型映射
- [x] 仅修复已确认的小松整机名称、分类、参数或详情翻译错误
- [x] 验证小松整机页面在十种语言中不回退或混杂
> 修复与验证：仅更新 `komatsumachineTranslations.ts` 及新增专用回归测试。补齐 11 台实际模型缺失的名称十语言映射，并为 12 台实际小松整机覆盖日语和德语名称、分类、参数与详情。修复日语/德语的 `Mini`、`Hydraulic Excavator`、`Motor Grader`、`bucket`、`blade` 等英文混杂，以及德语推土机刀铲误写为 `Schaufel` 的问题；使用 ミニ油圧ショベル、油圧ショベル、ブルドーザー、モーターグレーダー、ホイールローダー、Hydraulikbagger、Planierraupe、Motorgrader、Radlader、Planierschild 等术语。3 项回归测试通过，日语页面实测正常。未修改或删除小松零件、产品源数据、其他品牌、图片或页面组件。

## 小松零件十语言翻译检查与修复

- [x] 仅定位并审查小松零件独立名称、分类、描述和详情翻译文件及实际源键映射
- [x] 仅修复已确认的 22 个小松零件十语言名称、分类、描述或详情翻译错误
- [x] 验证小松零件列表与详情视图在十种语言中不回退或混杂
> 修复与验证：仅更新 `sparePartsTranslations_komatsu.ts`、`sparePartsDetails_komatsu.ts` 及新增专用回归测试。为 22 个实际小松零件补齐详情源键别名，修复中文字段遗留英文、德语 `Schauffel` 拼写、俄语驱动链轮语法和明确的术语错误；实际 5 个分类均有十语言映射。3 项回归测试通过，日语列表与详情弹窗已实测。未修改或删除小松整机、产品源数据、其他品牌、图片或页面组件。

## Shacman 整机十语言翻译检查与修复

- [x] 仅定位并审查 Shacman 整机独立翻译文件与实际模型映射
- [x] 仅修复已确认的 Shacman 整机名称、分类、参数或详情翻译错误
- [x] 验证 Shacman 整机页面在十种语言中不回退或混杂
> 修复与验证：仅更新 `shacmanmachineTranslations.ts` 及新增专用回归测试。9 台实际车型的十语言参数已与产品源数据对齐，修复了功率范围、总质量、驱动形式与搅拌车参数的不一致；日语和德语详情已改为与实际发动机、总质量、货箱/搅拌容量、液压系统、变速箱、驾驶室和燃油箱信息一致的本地化表述。4 项回归测试通过，日语页面实测正常。未修改或删除 Shacman 零件、产品源数据、其他品牌、图片或页面组件。

## Shacman 零件十语言翻译检查与修复

- [x] 仅定位并审查 Shacman 零件独立名称、分类、描述和详情翻译文件及实际源键映射
- [x] 仅修复已确认的 31 个 Shacman 零件十语言名称、分类、描述或详情翻译错误
- [x] 验证 Shacman 零件列表与详情视图在十种语言中不回退或混杂

> 修复与验证：仅更新 `sparePartsTranslations_shacman.ts`、`sparePartsDetails_shacman.ts` 及新增专用回归测试。为实际 31 个源零件名称保留并补齐独立别名映射，使列表名称、分类、描述与详情弹窗均命中十语言内容；`涡轮增压器总成`、柴油滤芯、空气滤芯和涡轮增压器进气管等实际源键不再回退。3 项专用回归测试通过，日语零件列表和机油滤清器详情弹窗已实际验证。未修改或删除 Shacman 整机、产品源数据、其他品牌、图片或页面组件。
...
> 修复与验证：仅更新 Sinotruk 独立整机映射、整机翻译函数、零件详情映射及两项专用回归测试。12 台实际整机的十语言名称、类型、参数与详情已按源数据重新对齐；日语已清除 `kW`、`GVW`、`m³`、`重型`、`重荷重` 等混杂，并将 HOWO 7 规范为 `大型トラック`。43 个实际零件的名称与分类映射完整；为空气滤芯、柴油粗滤器、起动机、转向节臂、传动轴、水泵、涡轮增压器及驱动轴节叉补齐详情别名，列表描述和弹窗不再返回默认文本；同时修正一处德语 `Fahrzeughebung` 拼写。2 个专用测试文件共 8 项测试通过，日语和德语整机页面、日语零件列表已实际核验。未修改或删除产品源数据、其他品牌、图片或页面组件。

## Toyota 整机与零件十语言翻译复核

- [x] 仅审查并修复 Toyota 整机独立翻译文件中实际车型的十语言名称、类型、参数和详情
- [x] 仅审查并修复 Toyota 零件独立翻译文件中实际零件的十语言名称、分类、描述和详情映射
- [x] 新增或扩展 Toyota 专用回归测试，并实际验证列表和详情视图

> 修复与验证：仅更新 Toyota 独立整机映射、整机翻译函数与 Toyota 专用回归测试。7 台实际车型的十语言名称、类型、规格与详情均重新以当前产品源数据为依据，恢复 4Runner、Fortuner、Land Cruiser 70、Prado、Hilux 4×4、Tacoma 4×4、Tundra 的实际发动机、功率、变速箱、座位、载重与油箱信息；日语规格与详情不再出现 `SUV`、`Pickup Truck`、`Diesel`、`Petrol`、`kW`、`kg`、`L` 等原始英文标签。Toyota 30 个实际零件的现有名称、分类和详情路由已审计完整；扩展的测试确认列表和详情弹窗均不回退 OEM 通用文本。两项专用测试共 7 项通过，日语整机、零件列表和机油滤清器详情弹窗均已实际核验。未修改或删除 Toyota 产品源数据、其他品牌、图片或页面组件。

## Isuzu 整机与零件十语言翻译复核

- [x] 仅审查并修复 Isuzu 整机独立翻译文件中实际发动机型号的十语言名称、类型、参数和详情
- [x] 仅审查并修复 Isuzu 零件独立翻译文件中实际零件的十语言名称、分类、描述和详情映射
- [x] 新增或扩展 Isuzu 专用回归测试，并实际验证列表和详情视图

> 修复与验证：仅更新 Isuzu 独立整机映射、整机翻译函数及 Isuzu 专用回归测试。4HK1、6HK1、6UZ1、6WG1 四台实际发动机的十语言名称、类型、规格与详情已依当前产品源数据重新对齐；日语使用“いすゞ”、日语发动机术语和本地化的排量、功率、马力、扭矩、燃料及排放单位，清除 `Diesel`、`Heavy-Duty`、`kW`、`HP`、`Nm`、`Fuel Type`、`Emissions Standard` 等原始标签。37 个 Isuzu 零件的现有独立名称、分类、列表描述与详情路由均已审计完整；扩展测试确认十语言内容不会回退 OEM 通用文本。两项专用测试共 7 项通过，日语整机、零件列表和机油滤清器详情弹窗均已实际核验。未修改或删除 Isuzu 产品源数据、其他品牌、图片或页面组件。

## Cummins 整机与零件十语言翻译复核

- [x] 仅审查并修复 Cummins 整机独立翻译文件中实际发动机型号的十语言名称、类型、参数和详情
- [x] 仅审查并修复 Cummins 零件独立翻译文件中实际零件的十语言名称、分类、描述和详情映射
- [x] 新增或扩展 Cummins 专用回归测试，并实际验证列表和详情视图

> 修复与验证：仅更新 Cummins 整机独立翻译文件、其独立候选映射、Cummins 专用测试与核验记录。7 台实际发动机的十语言名称、类型、规格和详情均已与当前产品源数据对齐；ISL8.9、ISM11、ISX15 的功率范围已分别校正为 180–210、250–280、330–380 千瓦。日语规格和详情使用本地化的排量、功率、马力、扭矩、燃料及排放标准表述，清除 `Diesel`、`Heavy-Duty`、`kW`、`HP`、`Nm`、`Fuel Type`、`Emissions Standard`、`Euro` 等原始英文标签。33 个实际零件的既有名称、分类、列表描述与详情路由已审计完整；扩展测试确认十语言内容均不回退 OEM 默认文本。两项专用测试共 8 项通过，日语整机、零件列表与涡轮增压器软管详情弹窗已实际核验。未修改或删除 Cummins 产品源数据、其他品牌、图片或页面组件。

## Weichai 整机与零件十语言翻译复核

- [x] 仅审查并修复 Weichai 整机独立翻译文件中实际发动机型号的十语言名称、类型、参数和详情
- [x] 仅审查并修复 Weichai 零件独立翻译文件中实际零件的十语言名称、分类、描述和详情映射
- [x] 新增或扩展 Weichai 专用回归测试，并实际验证列表和详情视图

> 修复与验证：仅更新 Weichai 整机独立翻译文件、其独立候选映射、Weichai 零件详情文件、专用测试与核验记录。WP6、WP10、WP12、WP13 的十语言名称、类型、规格和详情均按当前产品源数据对齐；WP10、WP12、WP13 的功率范围分别校正为 220–280、280–350、300–380 千瓦。日语规格和详情已清除 `Diesel`、`kW`、`HP`、`Nm`、`Euro` 及 `重型` 混杂；同时校正德语 `Betonmischer`、法语 `camions-pompes à béton`、意大利语 `betoniere` 等实际应用术语。30 个实际零件的名称、分类、列表说明与详情路由均已审计；日语详情中的中文品牌 `潍柴` 已替换为 `ウェイチャイ`，且不回退默认 OEM 文本。两项专用测试共 11 项通过，日语整机、零件列表和机油滤清器详情弹窗已实际核验。未修改或删除 Weichai 产品源数据、其他品牌、图片或页面组件。
