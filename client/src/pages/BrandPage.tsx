...
import { getTranslatedKomatsuMachineName, getTranslatedKomatsuMachineType, getTranslatedKomatsuMachineSpecs, getTranslatedKomatsuMachineDescription } from '@/data/komatsumachineTranslations';
import { getTranslatedIsuzuMachineName, getTranslatedIsuzuMachineType, getTranslatedIsuzuMachineSpecs, getTranslatedIsuzuMachineDescription } from '@/data/isuzumachineTranslations';
import { getTranslatedShacmanMachineName, getTranslatedShacmanMachineType, getTranslatedShacmanMachineSpecs, getTranslatedShacmanMachineDescription } from '@/data/shacmanmachineTranslations';
import { getTranslatedSinotruckMachineName, getTranslatedSinotruckMachineType, getTranslatedSinotruckMachineSpecs, getTranslatedSinotruckMachineDescription } from '@/data/sinotruckmachineTranslations';
import { getTranslatedToyotaMachineName, getTranslatedToyotaMachineType, getTranslatedToyotaMachineSpecs, getTranslatedToyotaMachineDescription } from '@/data/toyotamachineTranslations';
                  ...
                  >
                    ...
                    <div className="relative h-52 overflow-hidden bg-gray-100">
                      ...
                      <div
                        ...
                        style={{ color: 'oklch(0.68 0.18 42)', fontFamily: 'var(--font-display)' }}
                      >
                        {brand.id === 'shantui' ? getTranslatedShantuiMachineName(machine.model, language) : brand.id === 'sany' ? getTranslatedSanyMachineType(machine.name, language) : brand.id === 'liugong' ? getTranslatedLiuGongMachineName(machine.model, language) : brand.id === 'komatsu' ? getTranslatedKomatsuMachineName(machine.model, language) : brand.id === 'isuzu' ? getTranslatedIsuzuMachineName(machine.model, language) : brand.id === 'shacman' ? getTranslatedShacmanMachineName(machine.model, language) : brand.id === 'sinotruk' ? getTranslatedSinotruckMachineName(machine.model, language) : brand.id === 'toyota' ? getTranslatedToyotaMachineName(machine.model, language) : brand.id === 'cummins' ? getTranslatedCumminsMachineName(machine.model, language) : brand.id === 'weichai' ? getTranslatedWeichaiMachineName(machine.model, language) : machine.name}
                      </div>
                    </div>
                    ...
                    <div className="p-5">
                      <h3
                        ...
                        style={{ fontFamily: 'var(--font-display)', color: 'oklch(0.18 0.04 265)' }}
                      >
                        {brand.id === 'shantui' ? getTranslatedShantuiMachineName(machine.model, language) : brand.id === 'sany' ? getTranslatedSanyMachineTitle(`SANY ${machine.model}`, language) : brand.id === 'xcmg' ? getTranslatedXCMGMachineType(machine.name, language) : brand.id === 'liugong' ? getTranslatedLiuGongMachineName(machine.model, language) : brand.id === 'komatsu' ? getTranslatedKomatsuMachineName(machine.model, language) : brand.id === 'isuzu' ? getTranslatedIsuzuMachineName(machine.model, language) : brand.id === 'shacman' ? getTranslatedShacmanMachineName(machine.model, language) : brand.id === 'sinotruk' ? getTranslatedSinotruckMachineName(machine.model, language) : brand.id === 'toyota' ? getTranslatedToyotaMachineName(machine.model, language) : brand.id === 'cummins' ? getTranslatedCumminsMachineName(machine.model, language) : brand.id === 'weichai' ? getTranslatedWeichaiMachineName(machine.model, language) : `${brand.name} ${machine.model}`}
                      </h3>
                      <p className="text-sm font-semibold mb-2" style={{ color: 'oklch(0.45 0.02 265)' }}>
                        {brand.id === 'caterpillar' ? getTranslatedCATMachineType(machine.name, language) : brand.id === 'shantui' ? getTranslatedShantuiMachineType(machine.name, language) : brand.id === 'sany' ? getTranslatedSanyMachineType(machine.name, language) : brand.id === 'xcmg' ? getTranslatedXCMGMachineType(machine.name, language) : brand.id === 'liugong' ? getTranslatedLiuGongMachineType(machine.model, language) : brand.id === 'komatsu' ? getTranslatedKomatsuMachineType(machine.model, language) : brand.id === 'isuzu' ? getTranslatedIsuzuMachineType(machine.model, language) : brand.id === 'shacman' ? getTranslatedShacmanMachineType(machine.model, language) : brand.id === 'sinotruk' ? getTranslatedSinotruckMachineType(machine.model, language) : brand.id === 'toyota' ? getTranslatedToyotaMachineType(machine.model, language) : brand.id === 'cummins' ? getTranslatedCumminsMachineType(machine.model, language) : brand.id === 'weichai' ? getTranslatedWeichaiMachineType(machine.model, language) : machine.name}
                      </p>
                      ...
                      >
                        {brand.id === 'caterpillar' ? getTranslatedCATMachineSpecs(machine.model, language) : brand.id === 'shantui' ? getTranslatedShantuiMachineSpecs(machine.model, language) : brand.id === 'sany' ? getTranslatedSanyMachineSpecs(machine.model, language) : brand.id === 'xcmg' ? getTranslatedXCMGMachineSpecs(machine.specs, language) : brand.id === 'liugong' ? getTranslatedLiuGongMachineSpecs(machine.model, language) : brand.id === 'komatsu' ? getTranslatedKomatsuMachineSpecs(machine.model, language) : brand.id === 'isuzu' ? getTranslatedIsuzuMachineSpecs(machine.model, language) : brand.id === 'shacman' ? getTranslatedShacmanMachineSpecs(machine.model, language) : brand.id === 'sinotruk' ? getTranslatedSinotruckMachineSpecs(machine.model, language) : brand.id === 'toyota' ? getTranslatedToyotaMachineSpecs(machine.model, language) : brand.id === 'cummins' ? getTranslatedCumminsMachineSpecs(machine.model, language) : brand.id === 'weichai' ? getTranslatedWeichaiMachineSpecs(machine.model, language) : machine.specs}
                      </div>

                      {brand.id === 'caterpillar' || brand.id === 'shantui' || brand.id === 'sany' || brand.id === 'xcmg' || brand.id === 'liugong' || brand.id === 'komatsu' || brand.id === 'isuzu' || brand.id === 'shacman' || brand.id === 'sinotruk' || brand.id === 'toyota' || brand.id === 'cummins' || brand.id === 'weichai' ? (
                        <p className="text-sm leading-relaxed mb-4" style={{ color: 'oklch(0.45 0.02 265)' }}>
                          {brand.id === 'caterpillar' ? getTranslatedCATMachineDescription(machine.description, language) : brand.id === 'shantui' ? getTranslatedShantuiMachineDescription(machine.model, language) : brand.id === 'sany' ? getTranslatedSanyMachineDescription(machine.model, language) : brand.id === 'xcmg' ? getTranslatedXCMGMachineDescription(machine.description, language) : brand.id === 'liugong' ? getTranslatedLiuGongMachineDescription(machine.model, language) : brand.id === 'komatsu' ? getTranslatedKomatsuMachineDescription(machine.model, language) : brand.id === 'isuzu' ? getTranslatedIsuzuMachineDescription(machine.model, language) : brand.id === 'shacman' ? getTranslatedShacmanMachineDescription(machine.model, language) : brand.id === 'sinotruk' ? getTranslatedSinotruckMachineDescription(machine.model, language) : brand.id === 'toyota' ? getTranslatedToyotaMachineDescription(machine.model, language) : brand.id === 'cummins' ? getTranslatedCumminsMachineDescription(machine.model, language) : brand.id === 'weichai' ? getTranslatedWeichaiMachineDescription(machine.model, language) : null}
                        </p>
                      ) : (
                ...
                {filteredParts.map((part, idx) => (
                  ...
                  >
                    ...
                    <div className="relative h-40 overflow-hidden bg-gray-50 cursor-pointer hover:opacity-90 transition-opacity" onClick={() => { setSelectedPart(part); setPartDetailOpen(true); }}>
                      <img
                        src={part.image}
                        alt={brand.id === 'shantui' ? getTranslatedShantuiSparePartName(part.name, language) : brand.id === 'sany' ? getTranslatedSanySparePartName(part.name, language) : brand.id === 'xcmg' ? getTranslatedXcmgSparePartName(part.name, language) : brand.id === 'caterpillar' ? getTranslatedCaterpillarSparePartName(part.name, language) : brand.id === 'liugong' ? getTranslatedLiuGongSparePartName(part.name, language) : brand.id === 'komatsu' ? getTranslatedKomatsuSparePartName(part.name, language) : brand.id === 'isuzu' ? getTranslatedIsuzuSparePartName(part.name, language) : brand.id === 'shacman' ? getTranslatedShacmanSparePartName(part.name, language) : brand.id === 'sinotruk' ? getTranslatedSinotrukSparePartName(part.name, language) : brand.id === 'toyota' ? getTranslatedToyotaSparePartName(part.name, language) : brand.id === 'cummins' ? getTranslatedCumminsSparePartName(part.name, language) : brand.id === 'weichai' ? getTranslatedWeichaiSparePartName(part.name, language) : translatePartName(part.name)}
                    ...
                    <div className="p-4">
                      ...
                      <p className="text-xs leading-relaxed mb-3" style={{ color: 'oklch(0.5 0.02 265)' }}>
                        {brand.id === 'shantui' ? getTranslatedShantuiSparePartDescription(part.description, language) : brand.id === 'sany' ? getTranslatedSanySparePartDescription(part.description, language) : brand.id === 'xcmg' ? getTranslatedXcmgSparePartDescription(part.description, language) : brand.id === 'caterpillar' ? getTranslatedCaterpillarSparePartDescription(part.description, language) : brand.id === 'liugong' ? getTranslatedLiuGongSparePartDescription(part.description, language) : brand.id === 'komatsu' ? getTranslatedKomatsuSparePartDescription(part.description, language) : brand.id === 'isuzu' ? getTranslatedIsuzuSparePartDescription(part.description, language) : brand.id === 'shacman' ? getTranslatedShacmanSparePartDescription(part.description, language) : brand.id === 'sinotruk' ? getTranslatedSinotrukSparePartDescription(part.description, language) : brand.id === 'toyota' ? getTranslatedToyotaSparePartDescription(part.description, language) : brand.id === 'cummins' ? getTranslatedCumminsSparePartDescription(part.description, language) : brand.id === 'weichai' ? getTranslatedWeichaiSparePartDescription(part.description, language) : part.description}
                      </p>
